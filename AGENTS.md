...
This file provides guidance to Coding Agents like OpenCode or Claude Code when working with code in this repository.
...
- `opencode/` - OpenCode tool configuration and database settings
...
- `opencode/` contains tool-specific configurations (not deployed via Stow)
...
### OpenCode Integration (`opencode/` package)
- `.config/opencode/opencode.jsonc` - Main OpenCode configuration file
- `.config/opencode/tool/database.ts` - Database interface and query functionality
- `.config/opencode/tool/project.ts` - Project information and metadata handling
...
Note: `opencode/` directory contains tool-specific configurations and is not deployed via Stow.
...
- **opencode**: `.config/opencode/` with tool configuration files (not Stow-managed)
